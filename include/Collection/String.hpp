...
public:
   ...
   */
  Array<String> split(const Encoding::Regex &reg) const;
   ...
   */
  String replace(const Encoding::Regex &reg, const String &rep) const;
...
template <typename T, typename S> T deserialize(const S &s, usz &at) {
  if constexpr (HasDeserializeAt<T>::Value)
    ...
    return (T)Collection::InlineArray<u8>::deserialize(s, at);
  else {
    T obj;
    return obj;
  }
...
inline void writeVarLong(Collection::String &s, u64 v) { s.pushVarLong((long long)v); }
inline u64 readVarLong(const Collection::String &s, usz &at) {
  auto res = s.peekVarLong(at);
  if (res.error)
    return 0;
  at += res.bytes;
...
template <> struct FNVHasher<Collection::String> {
  static usz fnvHash(const Collection::String &s) {
