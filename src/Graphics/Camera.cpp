/**
 * @file Camera.cpp
 * @brief 3D Camera implementation for the Xi graphics module.
...
void Camera3::render(void *rtv, void *dsv, i32 w, i32 h) {
  ...
  Matrix4 viewRotation =
      multiply(rotateY(-getRotation().y), rotateX(-getRotation().x));
  Matrix4 viewTranslation =
      translate(-getPosition().x, -getPosition().y, -getPosition().z);
  Matrix4 view = multiply(viewTranslation, viewRotation);

  Matrix4 proj;
  if (isOrtho) {
    float halfW = (orthoScale * aspect) / 2.0f;
    float halfH = orthoScale / 2.0f;
    proj = ortho(-halfW, halfW, -halfH, halfH, clipStart, clipEnd);
  } else {
    float fovRad = fov * (3.14159f / 180.0f);
    proj = perspective(fovRad, aspect, clipStart, clipEnd);
  }

  if (shiftX != 0.0f || shiftY != 0.0f) {
    Matrix4 shiftMat = translate(shiftX, shiftY, 0.0f);
    proj = multiply(proj, shiftMat);
  }

  Matrix4 vp = multiply(view, proj);
  _renderRec(root, identity(), vp);
}
...
void Camera3::_ensureDepthBuffer(i32 w, i32 h) {
  if (pDSV) {
    ...
    if (pTex && pTex->GetDesc().Width == (uint32_t)w &&
        pTex->GetDesc().Height == (uint32_t)h)
      return;
  }
...
void Camera3::_renderRec(TreeItem *ti, Matrix4 p, const Matrix4 &vp) {
  ...
  if (r) {
    ...
    if (r->mesh && r->shader) {
      ...
      auto *pSamplerVar = srb->GetVariableByName(Diligent::SHADER_TYPE_PIXEL,
                                                 "g_Texture_sampler");

      if (pTexVar && r->surface) {
        void *srv = r->surface->deviceView(0);
        if (srv)
      ...
      gContext.drawMesh(r->mesh->_vb, r->mesh->_ib,
                        (usz)r->mesh->indices.size());
    }
  }
